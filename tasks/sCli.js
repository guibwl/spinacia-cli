...
const scriptsFileName = cp
  .execSync(`npm pack`, { cwd: path.join(packagesDir, 'spinacia-script') })
  .toString()
  .trim();
const scriptsPath = path.join(packagesDir, 'spinacia-script', scriptsFileName);
...
const spinaciaCliScriptPath = path.join(packagesDir, 'spinacia-cli', 'index.js');
cp.execSync(
